...
static int UI_MenuAnchorX( int x, int flags )
{
	float leftBias;

	if ( !(flags & QMF_LEFT_JUSTIFY) ) {
		return x;
	}

	/* Left-side menu strips should stay attached to the real left edge in widescreen,
	while central content keeps using the original 4:3 virtual area. */
	if ( x <= 140 ) {
		leftBias = (uis.scale > 0.0f) ? (uis.bias / uis.scale) : 0.0f;
		return (int)(x - leftBias);
	}
...
static void Text_Draw( menutext_s *t )
{
	...
	x = UI_MenuAnchorX( t->generic.x, t->generic.flags );
	y = t->generic.y;
...
static void Action_Init( menuaction_s *a )
{
	...
	// left justify text
	x = UI_MenuAnchorX( a->generic.x, a->generic.flags );
	a->generic.left   = x; 
	a->generic.right  = x + len*BIGCHAR_WIDTH;
