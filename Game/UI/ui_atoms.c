...
void UI_MouseEvent( int dx, int dy )
{
	...
	uis.cursorx += dx;
	{
		...
		leftBias = (uis.scale > 0.0f) ? (uis.bias / uis.scale) : 0.0f;
		minX = -leftBias;
		maxX = SCREEN_WIDTH + leftBias;

		if (uis.cursorx < minX)
			uis.cursorx = minX;
		else if (uis.cursorx > maxX)
			uis.cursorx = maxX;
	}
...
void UI_DrawHandlePicAspectFill( qhandle_t hShader ) {
	float screenAspect;
	float imageAspect;
	float drawW, drawH;
	float x, y;

	screenAspect = (float) uis.glconfig.vidWidth / (float) uis.glconfig.vidHeight;
	imageAspect = 4.0f / 3.0f;

	if ( screenAspect > imageAspect ) {
		drawW = (float) uis.glconfig.vidWidth;
		drawH = drawW / imageAspect;
		x = 0.0f;
		y = 0.5f * ( (float) uis.glconfig.vidHeight - drawH );
	}
	else {
		drawH = (float) uis.glconfig.vidHeight;
		drawW = drawH * imageAspect;
		x = 0.5f * ( (float) uis.glconfig.vidWidth - drawW );
		y = 0.0f;
	}

	trap_R_DrawStretchPic( x, y, drawW, drawH, 0, 0, 1, 1, hShader );
}
...
void UI_DrawBackPic(qboolean drawPic) {
	if (!drawPic) {
		...
		UI_DrawHandlePicAspectFill( uis.menuBackShader );
	}
}
...
void UI_Refresh( int realtime )
{
	...
	if ( uis.activemenu )
	{
		...
		{
			// draw the background
			if(!uis.hideBack){UI_DrawHandlePicAspectFill( uis.menuBackShader );}
