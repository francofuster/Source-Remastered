...
static qboolean cg_cameraPreviewLockedActive = qfalse;
static float cg_cameraPreviewBackup[4];
...
static void CG_OffsetThirdPersonView( void ) {
	...
	// LOCKED ON / CAMERA MENU LOCK PREVIEW
	{
		char previewBuf[16];
		qboolean previewLocked;

		trap_Cvar_VariableStringBuffer( "cg_cameraMenuPreviewLocked", previewBuf, sizeof(previewBuf) );
		previewLocked = (atoi(previewBuf) != 0);

		if (previewLocked && !cg_cameraPreviewLockedActive) {
			cg_cameraPreviewLockedActive = qtrue;
			cg_cameraPreviewBackup[0] = cg_thirdPersonAngle.value;
			cg_cameraPreviewBackup[1] = cg_thirdPersonHeight.value;
			cg_cameraPreviewBackup[2] = cg_thirdPersonRange.value;
			cg_cameraPreviewBackup[3] = cg_thirdPersonSlide.value;
		}
		else if (!previewLocked && cg_cameraPreviewLockedActive) {
			cg_cameraPreviewLockedActive = qfalse;
			cg_thirdPersonAngle.value = cg_cameraPreviewBackup[0];
			cg_thirdPersonHeight.value = cg_cameraPreviewBackup[1];
			cg_thirdPersonRange.value = cg_cameraPreviewBackup[2];
			cg_thirdPersonSlide.value = cg_cameraPreviewBackup[3];
		}

		if(ps->lockedTarget > 0 || previewLocked){
			if(ps->lockedTarget > 0 && !cg.lockonStart){cg.lockonStart = qtrue;}
