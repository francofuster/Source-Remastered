...
void Field_CustomDrawUniform(int spacing,field_t *edit, int x, int y, int width){
	int		len;
	int		drawLen;
	int		prestep;
	int		cursorChar;
	...
	Com_Memcpy( str, edit->buffer + prestep, drawLen );
	str[ drawLen ] = 0;
	SCR_DrawCustomStringUniform(spacing,x,y,str);
	if ( (int)( cls.realtime >> 8 ) & 1 ) {
		return;
	}
	if ( key_overstrikeMode ) {
	...
	str[0] = cursorChar;
	str[1] = 0;
	SCR_DrawCustomStringUniform(spacing, x + ( edit->cursor - prestep - i ) * spacing, y, str);
}
